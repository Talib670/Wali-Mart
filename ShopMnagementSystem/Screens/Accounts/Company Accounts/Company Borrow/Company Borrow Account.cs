...
{
    public partial class Company_Borrow_Account : Form
    {
        DataClasses19DataContext d = new DataClasses19DataContext();
        public Company_Borrow_Account()
        {
            InitializeComponent();
        }
        ...
        {
            {
                ...
                conn.Open();
                string str = "  Select * FROM [ShopManage].[dbo].[companyborrowbqayaamount] WHERE  [namecompany]= '" + comboBox1.Text + "'";
...
//SET [Grand Total] = '" + total.Text + "'";
                ...
                companybarrow t = new companybarrow();
                ...
                d.companybarrows.InsertOnSubmit(t);
                d.SubmitChanges();
                MessageBox.Show("Company Record Save SuccessFully...!!!", "Save", MessageBoxButtons.OK, MessageBoxIcon.Information);
...

                {
                    ...
                    string str = @" UPDATE [ShopManage].[dbo].[companyborrowbqayaamount]
SET [bqayaamount]= '" + bqayatextBox1.Text + "'  WHERE [namecompany] = '" + comboBox1.Text + "'";
                ...
                label5.Visible = false;
                {
                    SqlConnection conn = new SqlConnection(DecHelper.ConnectionString);
                    conn.Open();
                    string str = "  Select * FROM [ShopManage].[dbo].[companyborrowbqayaamount]";
                    SqlCommand cmd = new SqlCommand(str, conn);
                    SqlDataReader da = cmd.ExecuteReader();
                    while (da.Read())
                    {
        ...
        private void Company_Borrow_Account_Load(object sender, EventArgs e)
        {
            total.Visible = false;
            {
                grandtotal.Visible = false;
        ...
        {
            CompanyBorrowReportsForm1 cr = new CompanyBorrowReportsForm1();
            cr.ShowDialog();
        }
    }
}
